...
async function getAuthenticatedUser(authToken: string | undefined): Promise<UserRecord | null> {
...
const getDefaultSubjects = (stream: 'math' | 'science' | 'literature') => {
  if (stream === 'math') {
    return [
...
// 1. register
export const register = onRequest({ cors: true }, async (req, res) => {
  try {
...
// 3. forgotPassword
export const forgotPassword = onRequest({ cors: true }, async (req, res) => {
  try {
...
// 4. getStudyData
export const getStudyData = onRequest({ cors: true }, async (req, res) => {
  try {
...
// 5. saveStudyData
export const saveStudyData = onRequest({ cors: true }, async (req, res) => {
  try {
    const token = req.headers['x-auth-token'] as string | undefined;
    const user = await getAuthenticatedUser(token);
    if (!user) {
...
// 6. updateProfile
export const updateProfile = onRequest({ cors: true }, async (req, res) => {
  try {
    const token = req.headers['x-auth-token'] as string | undefined;
    const user = await getAuthenticatedUser(token);
    if (!user) {
...
// 7. whatsappLogs
export const whatsappLogs = onRequest({ cors: true }, async (req, res) => {
  try {
    const token = req.headers['x-auth-token'] as string | undefined;
    const user = await getAuthenticatedUser(token);
    if (!user) {
...
// 8. whatsappTestSend
export const whatsappTestSend = onRequest({ cors: true }, async (req, res) => {
  try {
    const token = req.headers['x-auth-token'] as string | undefined;
    const user = await getAuthenticatedUser(token);
    if (!user) {
...
export const whatsappSimulateAutoAll = onRequest({ cors: true }, async (req, res) => {
  try {
    const token = req.headers['x-auth-token'] as string | undefined;
    const user = await getAuthenticatedUser(token);
    if (!user) {
